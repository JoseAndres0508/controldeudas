...
import { activeDebts, creditorName, debtById, debtsByCreditor, fmtBase, fmtCRC, fmtDateLong, fmtMoney, lastBalance, toCRC } from '../utils.js';
...
function filteredMovements() {
  ...
  return DB.payments
    .filter(p => {
      ...
      const haystack = [d ? d.name : '', d ? creditorName(d) : '', p.note || ''].join(' ').toLowerCase();
...
export function renderReportes() {
  ...
  const movTotalCRC = movs.reduce((s, p) => {
    const d = debtById(p.debtId);
  ...
  html += `<div class="card">
    <div class="card-head">
      ...
      <span class="dim" style="font-size:13px">${movs.length} pago${movs.length === 1 ? '' : 's'} · ${fmtBase(movTotalCRC)}</span>
    </div>
    ...
    ${movs.length ? `<table><thead><tr><th>Fecha</th><th>Deuda</th><th class="hide-sm">Nota</th><th class="ta-r">Monto</th><th class="ta-r"></th></tr></thead><tbody>
      ${movs.map(p => {
        const d = debtById(p.debtId);
        return `<tr>
          ...
          <td class="ta-r num" style="color:var(--down)">${d ? fmtMoney(p.amount, d.currency) : ''}</td>
